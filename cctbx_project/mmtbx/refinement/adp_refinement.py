...
nm_master_params = iotbx.phil.parse("""\
  evecin = "evec.dat"
    .type  = str
  evalin = "eval.dat"
    .type = str
  ...
  eps = 1.e-6
    .type = float
  min_nm_group_size = 20
    .type = int
    .help = min number of atoms allowed per NM group
  verbose = True
    .type = bool
""")
...
#          print "gbr selections are based on nm selections"
       ...
       if(macro_cycle <= nm_params.stabilize_cycle):
         tools.split_u(fmodels.fmodel_xray().xray_structure, gbr_selections, offset)
       elif(macro_cycle <= nm_params.stabilize_cycle):
         u_eq = model.xray_structure_initial.extract_u_iso_or_u_equiv()
         nmtools.split_u(u_eq, fmodels.fmodel_xray().xray_structure, nm_selections, offset)
       else:
