...
selection = None
    .type = str
    .multiple = True
    .help = Selection for NM groups. Extracted from PDB file header or provided \
            below. It is the same as TLS groups when extracted from PDB file header.
extract_nm = None
    .type = bool
    .help = Fit NM parameters to B-factors of selected sets of atoms.
...
Purpose:
  - Fit NM parameters to B-factors of selected sets of atoms. Example:
      phenix.nm model.pdb extract_tls=ture selection='chain A' selection='chain B' evecin='eigenvectors.dat' evalin='eigenvalues.dat'
Questions and problems:
    phenixbb@phenix-online.org tluozhenwei@gmail.com"""
...
def run(args, command_name = "phenix.nm"):
  ...
  if(not command_line.options.silent):
    utils.print_header("NM tools", out = log)
  ...
  #
  if(not command_line.options.silent):
    utils.print_header("NM/TLS groups from PDB file header", out = log)
  ...
  if(len(tls_groups) == 0 and not command_line.options.silent):
    print >> log, "No NM/TLS groups found in PDB file header."
  else:
    for i_seq, tls_group in enumerate(tls_groups):
      tls_selections_strings.append(tls_group.selection_string)
      if(not command_line.options.silent):
        print >> log, "NM/TLS group %d: %s" % (i_seq+1, tls_group.selection_string)
        mmtbx.nm.tools.show_tls_one_group(tlso = tls_group, out = log)
        print >> log
  #
  ...
  if(ofn is not None):
    ...
    utils.write_pdb_file(
      ...
      out            = ofo)
    ofo.close()
    print >> log, "All done."   usage_fmt = "%s pdb_file [parameters:"
